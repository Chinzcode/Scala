...
object FPBasics {
  ...
  val trym = Person("Trym", "Interstellar")
  val trymStatement = trym.apply(4)
  val trymStatement_v2 = trym(4) //same
  ...
  //Scala already has a Function traits
  ...
  def main(args: Array[String]): Unit = {
    println(trymStatement)
    println(trymStatement_v2)
    println(six)
    println(six_v2)
    println(myCombinator("Scala", 99))
    println(myCombinator_v2("Scala", 99))
    println(superfunction(4, "Scala", 3.34, true))
  }
}
